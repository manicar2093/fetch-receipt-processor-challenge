...
func TestPaginator(t *testing.T) {
	...
	t.Run("creates a new page", func(t *testing.T) {
		var (
			...
			expectedPage              = paginator.Page[TestingModel]{
		...
		if err := expectedPage.SelectPages(db.Where("user_id = 2")); err != nil {
			t.Error(err)
			t.FailNow()
		}
	...
	t.Run("when there is a single page with few entries", func(t *testing.T) {
		...
		if err := expectedPage.SelectPages(db.Where("user_id = 3")); err != nil {
			t.Error(err)
			t.FailNow()
		}
	...
	t.Run("when first page has nothing", func(t *testing.T) {
		...
		if err := expectedPage.SelectPages(db.Where("user_id = 4")); err != nil {
			t.Error(err)
			t.FailNow()
		}
